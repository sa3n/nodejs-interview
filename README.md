...
<details>
    <summary>4. Что такое TDZ (Temporal Dead Zone)?</summary>

    Это область в коде, в которой переменная недоступна до тех пор, пока не инициализировано (присвоено) значение. Простирается от начала блока локальной области видимости то места присваивания переменной с значения.

    ```
    // TDZ для a
    console.log(a) // TDZ для a
    let a = 10  // Reference Error. TDZ для a заканчивается здесь
    ```
</details>

<details>
    <summary>5. Чем отличаются необъявленная переменная от переменных, содержащих `null` или `undefined`?</summary>

    Объявить переменную можно с помощтю ключевого слова let (var, const).

    ```
    let a = 10
    ```

    Необъявленная переменная — переменная, которой было присвоено значение без объявления с помощью let, const, var. В таком случае переменная объявляется в глобальной области видимости:
    ...
    Переменная, содержащая undefined — объявленная переменная, которой не назначено никакое значение.

    Переменная, содержащая null — должна быть явно установлена в это значение.
</details>

<details>
    <summary>6. Как определяется значение `this`?</summary>

    Порядок приоритетов связывания this (в порядке убывания):
    1. При вызове с new F() используется только что сконструированный объект, прототип которого установлен в F.prototype
    2. При вызове с call, apply, bind используется заданный объект
    3. Если функция является методом объекта, используется данный объект
    4. По-умолчанию: undefined в strict-режиме, глобальный объект в non-strict
</details>

<details>
    <summary>7. Что такое hoisting (поднятие)?</summary>

    Поднятие объявлений (не присваивания значений!) в начало области видимости. Поэтому мы можем использовать до объявления функции, объявленные как function declaration (объявления), но не function expression.

    В действительности физического перемещения объявления не происходит. На самом деле объявления переменных и функций попадают в память в процессе компиляции (смотри execution context object).
</details>

<details>
    <summary>8. Разница между `var`, `let`, `const`?</summary>
    
    Использование var не рекомендуется.

    * const создаёт константную переменную, значение которой не может быть присвоено повторно в отличии от let и var
    * при обращении к переменной до её объявленния: с let вызовет ReferenceError (переменная находится в TDZ), с var вернётся undefined (hoisting)
    * переменную можно повторно объявить с помощью var, повторное оъявление с let вызовет SyntaxError 
    * let в отличии от var не создаёт свойства глобального объекта при использовании на верхнем уровне 
    * var доступно из тела функции, даже если находится внутри блока (область видимости функции), область видимости let ограничена блоком:
...
<details>
    <summary>9. Область видимости функции и блочная область видимости</summary>

    Область видимости — концепция, определяющая доступность переменных; область программы, в рамках которой можно использовать переменную.

    Глобальная область видимости — в JavaScript это переменные, объявленные вне всех функций и блоков. В Node.js область верхнего уровня не является глобальной.

    Локальные области видимости в JavaScript:
    * область видимости функции — переменная доступна только внутри функции
    * область видимости блока — переменная, объявленная с помощью let или const доступна только внутри фигурных скобок (блока)
</details>

<details>
    <summary>10. 'use strict'</summary>

    Позволяет использовать более строгий вариант JavaScript.

    Наиболее важные особенности:
    * не игнорируются «тихие» ошибки
    * предотвращает случайное создание глобальных переменных (когда мы присваиваем переменной значение, которая не была объявлена с помощью let или const, эта переменная становится глобальной)
    * ссылка на this, являющееся null или undefined без strict mode автоматически становится ссылкой на global
    * запрещает синтаксис восьмеричной системы счислений
</details>

<details>
    <summary>11. Почему `0.3 == 0.2 + 0.1` возвращает false?<summary>

    Результатом вычисления 0.2 + 0.1 является 0.30000000000000004. 
...
<details>
    <summary>12. Что выведет следующий код?</summary>
    ...
    Результат: undefined и 2. 
    
    В первый вызов fn возвращает undefined так как this для функции = undefined. Второй вызов выглядит как arguments.fn(), поэтому this в этом случае соответствует arguments, а количество аргументов (this.length) равно двум (fn и 1).
</details>

<details>
    <summary>13. `const a = [1, 2, 3]`. Что произойдёт при `a[10] = 99`? Что выведет `console.log(a[5])`?</summary>

    Элементы с третьего по девятый будут empty. При обращении к ним вернётся undefined. 
    
    Разница между undefined и empty:
    * empty-слоты не являются перечислимыми (enumerable)
    * так как функция map работает только с перечислимыми значениями, empty не будут отображены, а undefined будут
    * оператор in не сработает для empty-слотов
</details>

<details>
    <summary>14. AJAX</summary>

    AJAX — подход к асинхронному неблокирующему (фоновому) обмену данными с сервером. 
    
    Стандартный подход:
    * браузер отправляет запрос сверверу с какой-либо страницы
    * сервер генерирует новую страницу и отправляет её браузеру

    AJAX:
    * браузер отправояет запрос серверу на определённую информацию
    * сервер возвращает только часть документа/необходимые данные
    * на клиенте не происходит перезагрузки страницы, вносятся только изменения с учётом полученной информации 

    Способы AJAX-запроса: XMLHttpRequest, Fetch API, Axios и др.
</details>
...
16. Типы ошибок в JavaScript
17. Чем отличается лексическая область видимости от динамической?
18. Обработка исключений
19. Оператор `!!`
20. Разница между spread- и rest-операторами
21. Проверка чётности числа без оператора `%`
22. Проверка существования подстроки в строке
23. Как проверить, является ли число конечным?
24. Чем отличается `isNaN()` от `Number.isNaN()`?
25. Garbage collector
26. Как обработать неперехваченные исключения в Node?
27. NODE_ENV
28. globals
...
<details>
    ...
    * Microtask Queue
    ...
    [Microtasks vs Macrotasks](https://stackoverflow.com/questions/25915634/difference-between-microtask-and-macrotask-within-an-event-loop-context)
</details>
...
![JOINS](https://learnsql.com/blog/learn-and-practice-sql-joins/2.png)
...
<details>
    <summary>2. `LEFT JOIN`</summary>

    Операция соединения (join) предназначения для выборки данных из двух таблиц и включения этих данных в результирующий набор. При необходимости соединения более двух таблиц, операция соединения применяется несколько раз. 

    Виды соединений:
    * Inner Join — внутреннее соединение
    * Outer Join (Left Join, Right Join, Full Join) — внешнее соединение (обязательно входят все строки одной из таблицы, или, в случае с Full Join — обеих таблиц)
    * Cross Join — декартово произведение

    LEFT JOIN используется для формирования таблицы из записей двух (или нескольких) таблиц. При этом возвращаются все записи из таблицы 1 и совпадающие записи из таблицы 2.
    
    Если нет совпадений, возвращаются все записи из левой таблицы (и 0 записей из правой). Столбцы правой таблицы при этом заполняются NULL
...
<details>
    <summary>3. `INNER JOIN`</summary>

    Используется для формирования таблицы из записей двух (или нескольких) таблиц, если есть совпадения по условию. Если условие истинно, строки попадают в результирующую таблицу. Если совпадений нет, возвращается пустая таблица.

    Самый распространённый тип объединения.

    Неявное использование:
    ```
    SELECT * FROM t1 WHERE t1.column = t2.column
    ```

    Явное использование (2 варианта):
    ```
...
<details>
    <summary>10. Типы данных MySQL</summary>

    Основные типы данных:

    String
    * CHAR(size) — Фиксированная длина. Строго size символов (можно ввести меньше, но память всё-равно резервируется для size символов), т. е. используется статическая malloc. До 255 символов;
    * VARCHAR(size) — Переменная длина. Не более, чем size символов (резервируется столько памяти, сколько символов было введено, т. е. использутеся динамическая malloc). До 65 536 символов;
    * TEXT(size) — Строка максимальной длины 65 536 Б;
    * ENUM(val1, val2, ...) — Может иметь одно строчное значение из списка возможных. Пример: ENUM ('MANAGEMENT', 'SALES');
    * SET(va1l1, val2, ...) — Объект, который может иметь 0 и более (до 64) различных значений из списка. Пример: ENUM ('MANAGEMENT', 'SALES'). Значениям во множеств будут сопоставлены двоичные номера: 1, 10, 100... Для выбора значений необходимо испоьзовать битовые маски (01 = SALES, 10 = MANAGEMENT, 11 = MANAGEMENT, SALES).

    Numeric
    * INTEGER — Целое;
    * FLOAT — Вещественное, 4 байта;
    * DOUBLE — Вещественное, 8 байт;
    * DECIMAL(size, decimalDigits) — Пример: DECIMAL(5,2) = 999.99;
    * BIT(size) — числа до 2^size. При выходе за границы диапазона происходит округление до 2^size - 1 (наибольшего возможного числа).

    Date and time
    * DATE — Дата в формате YYYY-MM-DD;
    * TIME — Время в формате HH:MM:SS;
    * DATETIME — Дата и время в формате YYYY-MM-DD HH:MM:SS;
    * YEAR — Год в четырёхзначном формате;
    * TIMESTAMP — Дата и время в формате YYYY-MM-DD HH:MM:SS, хранимое как количество секунд с Unix epoch.
    
    Logical
    * BOOL = BOOLEAN — 0 = FALSE, а все ненулевые значения расцениваются как TRUE.
    
    Blob — различные типы для хранения бинарных данных.
</details>
...
<details>
    <summary>15. Разница между `CHAR` и `VARCHAR`</summary>

    * CHAR(size) — Фиксированная длина. Строго size символов (можно ввести меньше, но память всё-равно резервируется для size символов), т. е. используется статическая malloc. До 255 символов;
    * VARCHAR(size) — Переменная длина. Не более, чем size символов (резервируется столько памяти, сколько символов было введено, т. е. использутеся динамическая malloc). До 65 536 символов;

    Char удобно использовать для строчных значений известной фиксированной длины, например: 
	* аббревиатуры штатов: CA, NY
	* пол: F, M
</details>

<details>
    <summary>16. Разница между `DECIMAL`, `FLOAT`, `DOUBLE`</summary>

    * Вычисления в Decimal — точные, а в Float и Double — приблизительные.
    * Float и Double могут хранят большие числа, используя меньше места.
</details>

17. `CREATE FUNCTION`
18. `CREATE PROCEDURE`
19. Разница между внешними ключами и объединениями
20. `COALESCE`
21. Транзакции
